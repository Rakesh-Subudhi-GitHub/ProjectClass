...
public class ContactController {
	...
	public String saveContact(@Valid Contact cont,BindingResult result, Model model)
	{
		
		//validate the contact form if it mistake then redirect return back to it
		if(result.hasErrors())
		{
			return "contact";
		}
		
		if(contactService.isContactExist(cont))
		{
			//contact is there so duplicate contact enter
			model.addAttribute("errMsg","Contact is already exist Dulicate contact is not Insert So... Try Again ");
		}
		else {
